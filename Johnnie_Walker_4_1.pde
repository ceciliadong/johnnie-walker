...
void getUltrasonicSensorValues() {
  ...
  printUltrasonics();
  // parse the side sensor values to be the vertical distance to object
  //parse(0, 1);
  //parse(4, 5);
  //printUltrasonics();
  Serial.println();
...
void parse(int m, int n) {
  long int lp[2] = {ultrasonicSensorValues[m], ultrasonicSensorValues[n]};
  long int b = 26; // the distance between two sensors
  long int a = lp[0] - lp[1];
  ultrasonicSensorValues[m] = roughSqrt(sq(lp[0])*sq(b)/(sq(a)+sq(b)));
  ultrasonicSensorValues[n] = roughSqrt(sq(lp[1])*sq(b)/(sq(a)+sq(b)));
}
...
void establishUltrasonicBaseline() {
  ...
  rightBaseline = max((ultrasonicSensorValues[4] + ultrasonicSensorValues[5]) / 2, minDis);
}
  ...
  if(left < 85 && right < 85) {
    if(leftBaseline < maxDis || rightBaseline < maxDis) {
      if(leftBaseline < rightBaseline) {
        setMotorSpeed(baseSpeed+7, baseSpeed-7);
      } else {
        setMotorSpeed(baseSpeed-7, baseSpeed+7);    
      }
      delay(240*baseSpeed);
...
void joystickControl() {
  ...
  int yDir = y >= 50 ? 1 : -1;
  int xDir = x > 50 ? 1 : -1;
  int basev = baseSpeed * yDir * (abs(y-50) > 10) + (y - 50) / 10; // base speed
...
void loop() {
  ...
  sendCondition();
  if(mode == 1) modeAuto();
  else if(mode == 0) modeJs();
  // if mode == 3, it's stopped, waiting for resume
}
