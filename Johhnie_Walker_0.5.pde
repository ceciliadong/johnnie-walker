...
void updateBoundary() {
  ...
  int tmp;
  for(int i = 1; i < 3; i++) {
    tmp = ((sensorValues[2*i] + sensorValues[2*i+1])/2 + boundary[i])/2;
    if(tmp > 200 && tmp < 600 && (tmp-boundary[i] < 50) && (tmp-boundary[i] > -50))
      boundary[i] = tmp;
  }
}
...
void modeWall() {
  ...
  if(fSen > 550) frontRightWallTurn();
...
void frontRightWallTurn() {
  ...
  do {
    ...
    fs = sensorValues[1];
    MoveBackward();
  } while(fs > boundary[2]); // using right boundary
  do {
    getSensorValues();
    rf = sensorValues[5];
    setMotorSpeed(6, 2);
...
void setup()
{
  ...
  initIRSensors();
  initJoystick();
  timer.setInterval(1000, repeatMe);  //In milliseconds
}
...
void loop()
{
 ...
 // features joystick as the highest priority
 if(mode == 1) {
   boolean isJs = false, preJs = true;
   getJoyStickValue();
   int x = JoyStick_X_VAL;
   int y = JoyStick_Y_VAL;
   Serial.print(x);
